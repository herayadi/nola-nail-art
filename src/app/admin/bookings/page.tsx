    ...
    <div className={styles.container}>
      <header className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>Overseeing every brushstroke and appointment in the studio.</p>
        </div>
      ...
      <div className={styles.filters}>
        <Link href="/admin/bookings" className={`${styles.filterBtn} ${!status ? styles.filterBtnActive : ""}`}>
          ALL SESSIONS
        </Link>
        <Link href="/admin/bookings?status=PENDING" className={`${styles.filterBtn} ${status === "PENDING" ? styles.filterBtnActive : ""}`}>
          PENDING
        </Link>
        <Link href="/admin/bookings?status=CONFIRMED" className={`${styles.filterBtn} ${status === "CONFIRMED" ? styles.filterBtnActive : ""}`}>
          CONFIRMED
        </Link>
        <Link href="/admin/bookings?status=COMPLETED" className={`${styles.filterBtn} ${status === "COMPLETED" ? styles.filterBtnActive : ""}`}>
          ARCHIVED
        </Link>
      </div>
                ...
                <tr key={booking.id}>
                  ...
                  <td>
                    <div className="flex flex-col">
                      ...
                      <span className={`text-[9px] font-bold tracking-tighter uppercase ${booking.paymentStatus === "PAID" ? "text-success" : "text-error"}`}>
